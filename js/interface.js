...
import {
  ...
  applyBlur,
  updateStarPoints,
  updateStarOuterRadius,
  updateStarInnerRadius,
  updatePolygonSides,
  updatePolygonRadius,
} from "./shapes.js";
import { updateBackgroundColor, updateBackgroundOpacity } from "./canvas.js";
import {
  setBrushObjectStrokeColor,
  setBrushObjectStrokeWidth,
  getObjectStrokeColor,
  syncVectorBrush,
} from "./vector-brush.js";
...
export function initInterface() {
  ...
  if (fillPicker) {
    ...
    fillPicker.parentElement.style.backgroundColor =
      state.defaults.fill === "transparent" ? "#ffffff" : state.defaults.fill;
    fillPicker.value =
      state.defaults.fill === "transparent" ? "#ffffff" : state.defaults.fill;
  ...
  if (swapBtn) {
    swapBtn.addEventListener("click", () => {
      ...
      if (fillPicker) {
        ...
        fillPicker.parentElement.style.backgroundColor =
          newFill === "transparent" ? "#ffffff" : newFill;
      }
      if (strokePicker) {
      ...
      if (fillStrike)
        fillStrike.style.display = newFill === "transparent" ? "block" : "none";
      if (strokeStrike)
        strokeStrike.style.display = newStrokeWidth === 0 ? "block" : "none";
...
export function updateLayersList() {
  ...
  function renderObjects(objList, container, depth = 0, parentGroup = null) {
    objList.forEach((obj, index) => {
      ...
      layerItem.dataset.id =
        obj.uid || (obj.uid = Math.random().toString(36).substr(2, 9));

      if (
        state.canvas.getActiveObjects().includes(obj) ||
        state.canvas.getActiveObject() === obj ||
      ...
      layerItem.addEventListener("click", (e) => {
        ...
        const siblings = obj.group
          ? obj.group.getObjects()
        ...
        if (
          isShift &&
          state.lastSelectedLayerObject &&
          state.lastSelectedLayerObject.group === obj.group
        ) {
          ...
          if (idx1 !== -1 && idx2 !== -1) {
            ...
            if (!obj.group) {
              if (range.length > 1) {
                state.canvas.discardActiveObject();
                const sel = new fabric.ActiveSelection(range, {
                  canvas: state.canvas,
                });
          ...
          if (!obj.group) {
            ...
            if (isSelected) {
              ...
              if (newSet.length === 1) {
                ...
                const sel = new fabric.ActiveSelection(newSet, {
                  canvas: state.canvas,
                });
                state.canvas.setActiveObject(sel);
              }
...
function getLayerIcon(obj) {
  ...
  if (
    obj.type === "polygon" ||
    (obj.type === "path" && obj.shapeType === "polygon")
  ) {
...
function getLayerName(obj) {
  ...
  if (obj.type === "polygon")
    return obj.points.length === 5 ? "Star" : "Polygon";
  if (obj.type === "line") return "Line";
  if (obj.type === "polyline") return "Polyline";
  if (obj.type === "path")
    return obj.shapeType === "polygon" ? "Custom Shape" : "Path";
...
export function updatePropertiesPanel() {
  ...
  if (
    ...
    html += `
            ...
            <div class="property-group">
                <label class="property-label">Character spacing</label>
                <div class="range-container">
                    <input type="range" min="-100" max="1000" step="10" value="${activeObj.charSpacing || 0}"
                           oninput="updateObjectProperty('charSpacing', parseInt(this.value)); this.nextElementSibling.textContent = this.value">
                    <span class="range-value">${activeObj.charSpacing || 0}</span>
                </div>
            </div>
  ...
  if (activeObj.type === "image") {
    const imgStrokeWidth =
      activeObj.imgStrokeWidth !== undefined
        ? activeObj.imgStrokeWidth
    ...
    html += `
            ...
            <div class="property-group">
                <div style="display: flex; align-items: center; justify-content: space-between; padding-right: 5px;">
                    <label class="property-label">Grayscale</label>
                    <input type="checkbox" title="Enable Grayscale"
                           ${activeObj.isGrayscale ? "checked" : ""}
                           onchange="updateImageGrayscale(this.checked)">
                </div>
            </div>
            <div class="property-separator"></div>
        `;
              ...
              activeObj.shadow
                ? (() => {
                    ...
                    if (activeObj.shadow.color) {
                      const match = activeObj.shadow.color.match(
                        /rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/,
                      );
                      if (match && match[4] !== undefined)
                        shadowOpacity = parseFloat(match[4]);
                    }
                    return `
                <div class="property-group">
...
export function clearPropertiesPanel() {
  document.getElementById("propertiesPanel").innerHTML =
    '<p class="empty-state">Select a layer to edit properties</p>';
}
...
export function updateObjectProperty(property, value) {
  ...
  const applyToObj = (obj, prop, val) => {
    ...
    if (
      (obj.type === "image" || obj.type === "text" || obj.type === "i-text") &&
      (prop === "fontSize" ||
        prop === "fontWeight" ||
        prop === "fontStyle" ||
        prop === "text")
  ...
  if (property === "fill") {
    ...
    const fillInput = document.querySelector(
      `input[oninput*="updateObjectProperty('fill'"]`,
    );
  ...
  if (property === "fill" || property === "stroke") {
    const inputs = document.querySelectorAll(
      `input[oninput*="updateObjectProperty('${property}'"]`,
    );
    inputs.forEach((input) => {
      ...
      if (
        input.type === "color" &&
        input.parentElement.classList.contains("color-preview")
      ) {
...
export function updateImageGrayscale(isGrayscale) {
  const activeObj = state.canvas.getActiveObject();
  if (!activeObj || activeObj.type !== "image") return;

  activeObj.filters = activeObj.filters || [];
  const grayscaleIndex = activeObj.filters.findIndex(
  ...
  if (isGrayscale) {
    ...
    if (grayscaleIndex > -1) {
      activeObj.filters.splice(grayscaleIndex, 1);
    }
  }

  activeObj.isGrayscale = isGrayscale;
...
export function updateShadowProperty(property, value) {
  ...
  if (property === "intensity") {
    ...
    let currentIntensity =
      activeObj.shadowIntensity !== undefined ? activeObj.shadowIntensity : 1;
  ...
  if (property !== "intensity" && property !== "color") {
    const inputs = document.querySelectorAll(
      `input[oninput*="updateShadowProperty('${property}'"]`,
    );
    inputs.forEach((input) => {
      if (document.activeElement !== input) {
        input.value = value;
...
export function alignSelected(direction) {
  ...
  activeObj.getObjects().forEach((obj) => {
    ...
    const absCorners = localCoords.map((p) =>
      fabric.util.transformPoint(p, groupMatrix),
    );
